...
# The Research Planner software architecture
## Project Folder - research_planner. 
This is the main folder that contains the files that configure the entire Django project. It usually has the same name as the Django project. In this case it is called **research_planner** and it contains the following files:

  - manage.py: A command-line utility script used to interact with the project.
  - settings.py: Configuration file for the project's settings, including database settings, middleware, installed apps, etc.
  - urls.py: URL configuration for the project, mapping URLs to views.
  - wsgi.py: Entry point for the project's WSGI-compatible web server.
   
## Application Folder - jobs 
This folder represents a specific Django application within the project. It can have the same name as your application or something related to its purpose.  In this case it is called **jobs** and it contains the following files and folders:

  - admin.py: Configuration file for the Django admin interface, where you can manage models and their data.
  - forms.py: Defines structured and validated forms for collecting user input.
  - models.py: Defines the application's data models.
  - views.py: Contains the logic for handling HTTP requests and generating responses.
  - migrations/: Folder containing database migration files.
  - modules/: Folder containing modules that contain helper functions used in views.py
  - static/: Folder for static files like CSS, JavaScript, images, etc.
  - templates/: Folder for HTML templates used by the views. The **jobs** sub-folder contains templates used by the **jobs** application. The **admin** sub-folder contains templates used by the built-in Django admin function.
...
To run all the tests, open a command prompt in the root of the research planner project and issue the following command,
